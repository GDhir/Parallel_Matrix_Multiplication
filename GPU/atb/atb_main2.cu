...
int main( int argc, char* argv[] ){
  ...
  double *h_A, *h_B, *h_C, *h_D, *h_Cref, *d_A, *d_B, *d_C, *d_D;
  int i,j,k,Ni,Nj,Nk;
  ...
  dim3 grid( gridsz, 1 );
    ...
    h_D = (double *) malloc(sizeof(double)*gridsz);
    for( int i = 0; i < gridsz; i++ ) {
        h_D[i] = 0;
    }
  cudaMalloc(&d_D, gridsz*sizeof(double));
  cudaMemcpy(d_D, h_D, gridsz*sizeof(double), cudaMemcpyHostToDevice);
  ...
  for(int trial=0;trial<5;trial++)
  {
    ...
    // cudaMemcpy(d_C, h_C, Ni*Nj*sizeof(double), cudaMemcpyHostToDevice);
...
//   cudaDeviceSynchronize();
   ...
   fprintf(fptr, "<Ni=%d,Nj=%d,Nk=%d>: Trial %d: GFLOPS: %.2f\n",Ni,Nj,Nk,trial,2.0e-6*Ni*Nj*Nk/elapsedTime);
